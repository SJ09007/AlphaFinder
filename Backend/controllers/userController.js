...
const registerUser = async (req, res) => {
    try {
        ...
        if(existinguser && !existinguser.isdeleted) {
            await sendEmail(process.env.SMTP_EMAIL, req.body.email, "User already exists", "User already exists");
        ...
        if(existinguser && existinguser.isdeleted) {
            ...
            await sendEmail(process.env.SMTP_EMAIL, req.body.email, "User registered again", "User registered again");
        }
        ...
        await sendEmail(process.env.SMTP_EMAIL, req.body.email, "User registered", "User registered ");
...
const login = async (req, res) => {
    try {
        ...
        res.status(200).cookie("access_token", token, { httpOnly: true, sameSite: "none", secure: true }).json({...others, token});
