const ClaimedItem = require("../models/ClaimsModels");
const Item = require("../models/itemsModel");

const addClaim = async (req, res) => {
    try {
        item = await Item.findById(req.params.id);
        if(!item) {
            return res.status(404).json("Item not found");
        }
        if(item.status == "claimed" || item.status == "reported") {
            return res.status(400).json("Item has already been claimed");
        }
        if(item.postedby != req.user._id) {
            return res.status(400).json("You can only claim your items");
        }

        // find all persons who have claimed the item
        const claims = await ClaimedItem.find({ item: req.params.id });


        if(claims.users.includes(req.user._id)) {
            return res.status(400).json("You have already claimed this item");
        }
        if(!claims.users.includes(req.user._id)) {
            claims.users.push(req.user._id);
            await claims.save();
        }


        if(!claims){

            const claimedItem = await ClaimedItem.create({item: item._id, users: [req.user._id]});
            res.status(200).json(claimedItem);
        }
        
    } catch (err) {
        console.log(err);
        res.status(500).json(err);
    }
}

const getItemClaims = async (req, res) => {
    try {
        const claims = await ClaimedItem.find({item: req.params.id});
        res.status(200).json(claims);
    } catch (err) {
        console.log(err);
        res.status(500).json(err);
    }
}

module.exports = {addClaim, getItemClaims};